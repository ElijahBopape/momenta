...
### Milestone 7 — status: done

Hardening and launch readiness: RLS audit, DB-backed rate limiting, an accessibility pass, OG/share metadata, and branded error boundaries.

Built:
- **RLS audit** across every table via direct `pg_tables`/`pg_policies`/`information_schema` queries (not just reading the migration files) — found one real gap: `_migrations` (created by `scripts/migrate.mjs`) had RLS disabled, letting any anon/authenticated API key read migration filenames via PostgREST. Not sensitive data, but sloppy; fixed in `0006_migrations_table_rls.sql`. Everything else already had correct owner-only policies, and the broad default `anon`/`authenticated` grants Supabase applies by default are safe specifically because RLS is the actual enforcement gate.
- **DB-backed rate limiting** (`src/lib/rate-limit.ts`) — a fixed-window counter table (`0007_rate_limit.sql`, RLS enabled, zero policies, service-role only) rather than adding an external service like Upstash, keeping the free-tier-only constraint intact. Applied to the public accept/decline actions (per-token and per-IP buckets) and to Find a Spot search (per-user bucket).
- **Automated accessibility pass** using `@axe-core/playwright` end-to-end across the real sender→recipient flow, not a subjective manual review. Two real, confirmed findings, both fixed: missing `<main>` landmark on the four auth pages and several standalone pages (`(auth)/layout.tsx`, `i/[token]/not-found.tsx`, `i/[token]/already-answered.tsx`, `auth/auth-code-error/page.tsx`, `error.tsx`, `public-invitation-flow.tsx`); missing `<h1>` on the public "ask" screen and the sender's invitation detail page (both added as `sr-only`, since the visual heading is carried by the mascot card instead).
- **OG images and share metadata** — a `next/og`-generated 1200×630 branded image at `src/app/opengraph-image.tsx`, plus `metadataBase` added to the root layout (was missing; OG URLs were silently resolving to `localhost` even in production).
- **Branded error boundaries** — root `not-found.tsx`, `error.tsx`, and `global-error.tsx` (the latter intentionally minimal/inline-styled with no shared component imports, since it has to survive a failure in the root layout itself).

One real bug found late, during accessibility verification, worth recording in detail because the investigation was initially misdirected: a comprehensive end-to-end a11y scan kept reporting a missing `<main>` landmark specifically on the public flow's datetime/celebrate screens, even though the source clearly had a single unconditional `<main>` wrapper around all screens, and every isolated reproduction (direct DOM query, curl of server HTML, a stepwise trace through screen transitions on a directly-DB-inserted test invitation) showed `<main>` present and stable. The actual cause: the share link shown in the app is built from `NEXT_PUBLIC_SITE_URL`, which wasn't set in `.env.local` — so both the sender's copy-link button and the invitation detail page fell back to a hardcoded production URL (`https://momenta-web.vercel.app`). The comprehensive test copied that link verbatim, meaning it was actually scanning the *live production site* (running old, unfixed code) rather than the local build under test — while the narrower isolated reproductions happened to construct their own `localhost` URLs directly and so never hit the bug. Confirmed by rewriting the captured share URL's host to `localhost` mid-test: violations dropped to zero immediately. Fixed at the root rather than patching the test: the client-side share link (`invitation-builder.tsx`) now uses `window.location.origin`, always correct regardless of env config; the server-side one (`invitations/[id]/page.tsx`) and `metadataBase` now both use the existing `getSiteURL()` helper (`src/lib/site-url.ts`), which already correctly falls back through Vercel's system env vars to `localhost` — it just wasn't being used consistently.

Verified end-to-end against a clean `npm run build` + `npm run start` (i.e. the same artifact that ships): a full axe-core scan across create → sent confirmation → sender detail → public name/ask/activity/datetime/celebrate screens returned zero violations; 404 and OG image routes returned correct status/content-type; `npm run lint` clean. All temporary debug scripts and test data (a throwaway confirmed test user and its invitations) created during the investigation were deleted afterward.
